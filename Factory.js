...
(function () {
  ...
  function playerDRDD() {
    ...
    // ── ROBE / COAT (back panel + shoulder pad + lapels) ─────────────────
    var robeBack = mk(new THREE.BoxGeometry(0.98, 1.05, 0.18), mNavy);
    robeBack.position.set(0, 0.20, -0.38);
    g.add(robeBack);
    [-0.50, 0.50].forEach(function (sx) {
      var sh = mk(new THREE.BoxGeometry(0.24, 0.18, 0.72), mNavy);
      sh.position.set(sx, 0.68, 0);
      g.add(sh);
    });
    // Lapels — now rounded slightly using CapsuleGeometry
    [-1, 1].forEach(function (side) {
      var lap = mk(new THREE.CapsuleGeometry(0.09, 0.85, 6, 10), mNavy);
      lap.position.set(side * 0.50, 0.20, 0.12);
  ...
  function characterMicFlexDurag() {
    ...
    var muzzleM= mat(0x88aaff, {roughness:0.4});
    var gripM  = mat(0x1144aa);

    // Body — smooth lathe torso instead of a plain box (this is the mesh
    // the rage-gauge color-tint swaps out, so it must stay sizeable/visible)
    var body = latheTorso([
      [0.20,-0.42],[0.30,-0.18],[0.34,0.10],[0.33,0.30],[0.22,0.42],
    ], bodyM, 16);
    ...
    // Hair — rounded cap instead of flat slab
    var hair = mk(new THREE.SphereGeometry(0.28, 14, 10, 0, Math.PI*2, 0, Math.PI*0.45), hairM);
    hair.position.y = 1.30;
    g.add(hair);

    // Sunglasses — rounded bar
    var glasses = mk(new THREE.CapsuleGeometry(0.05, 0.34, 4, 8), glassM);
    glasses.rotation.z = Math.PI / 2;
    glasses.position.set(0, 1.10, 0.25);
    g.add(glasses);

    // Pants — smooth tapered capsule legs
    [-0.18, 0.18].forEach(function (lx) {
      var leg = limb(0.14, 0.13, 0.5, pantM, 12);
      leg.position.set(lx, -0.16, 0);
      g.add(leg);
    });

    // Shoes — smooth capsule
    [-0.18, 0.18].forEach(function (sx) {
      var shoe = mk(new THREE.CapsuleGeometry(0.13, 0.20, 6, 10), shoeM);
      shoe.rotation.z = Math.PI / 2;
      shoe.position.set(sx, -0.44, 0.04);
      g.add(shoe);
    });

    // Shotdipe — chunky sawn-off shotgun, smooth capsule barrel
    var gunBody = mk(new THREE.CapsuleGeometry(0.13, 0.30, 6, 10), gunM);
    gunBody.rotation.z = Math.PI / 2;
    gunBody.position.set(0.44, 0.36, 0.18);
    g.add(gunBody);
    var barrel = limb(0.08, 0.08, 0.64, barrelM, 10);
    barrel.rotation.z = Math.PI / 2;
    barrel.position.set(0.44, 0.44, 0.66);
    g.add(barrel);
    var muzzle = mk(new THREE.SphereGeometry(0.10, 10, 8), muzzleM);
    muzzle.position.set(0.44, 0.44, 1.00);
    g.add(muzzle);
    var grip = mk(new THREE.CapsuleGeometry(0.06, 0.10, 4, 8), gripM);
    grip.position.set(0.44, 0.16, 0.14);
    g.add(grip);
